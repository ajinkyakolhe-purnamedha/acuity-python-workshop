...
def import_csv(csv_path: str | Path, client: APIClient) -> dict[str, Any]:
    ...
    with csv_path.open() as fh:
        ...
        for row_no, row in enumerate(reader, start=2):  # row 1 is the header
            # TODO 1 — VALIDATE the row. On ValidationError, append to
            #          validation_errors ({"row", "input", "errors": exc.errors()})
            #          and `continue` — the row must NOT reach the API.
            #   try: payload = ProductCreate.model_validate(row)
            #   except ValidationError as exc: ...; continue

            # TODO 2 — CREATE via the API. On APIError, append to api_errors
            #          ({"row", "input", "status": exc.status_code, "detail": exc.detail})
            #          and `continue`.
            #   try: product = client.create_product(payload)
            #   except APIError as exc: ...; continue

            # TODO 3 — SUCCESS: created.append(product.model_dump())
            ...
...
def main(argv: list[str] | None = None) -> int:
    ...
    # TODO: fail fast if the server is unreachable — ping health, return 2 on error.
    #   try: client.health()
    #   except Exception as exc: logger.error(...); return 2
